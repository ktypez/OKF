...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .legend-items {
    display: none; padding: 4px 12px 8px; gap: 4px; flex-wrap: wrap;
  }
  .legend-items.open { display: flex; }
  .legend-item {
...
function toggleLegend() {
  const items = document.getElementById('graphLegend');
  items.classList.toggle('open');
  const btn = document.querySelector('.legend-toggle');
  btn.innerHTML = items.classList.contains('open') ? 'Legend \\u25BC' : 'Legend \\u25B6';
}
...
function setFilter(f) {
  ...
  document.querySelectorAll('.sidebar-nav button').forEach(b => {
    if (b.textContent.toLowerCase() === f.toLowerCase() || (f.startsWith('project:') && b.textContent.toLowerCase() === f.replace('project:', '').toLowerCase())) {
